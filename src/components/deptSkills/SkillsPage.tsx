...
import { SkillRow } from "./deptRowDisplay";
import { type Department, type Skill } from "../../types/user";
...
export default function SkillsPage() {
  ...
  const [openModal, setOpenModal] = useState<
    false | { mode: "create" } | { mode: "edit"; item: Skill }
  >(false);
  ...
  const fetchAll = async (pageNumber = 1, search = query) => {
    try {
      ...
      const [dRes, sRes] = await Promise.all([
        deptSkillsAPI.getAlldept({ limit: 1000 }),
        deptSkillsAPI.getAllskills({
          page: pageNumber,
          limit,
      ...
      const ds: Department[] =
        dRes.data?.data || dRes.data?.items || dRes.data?.departments || [];

      const data = sRes.data;
      const list: Skill[] = data?.data || data?.items || data?.skills || [];
      const total =
        data?.totalItems ?? data?.total ?? data?.count ?? list.length;

      const withDept = list.map((x: any) => ({
        ...x,
        department: x.departmentId?._id ? x.departmentId : undefined,
      }));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deptSkillsAPI.Deleteskills(id);
      MySwal.fire({
        icon: "success",
        title: "Deleted",
        toast: true,
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-4 mb-4">
        <button
          ...
          className="flex items-center gap-2 bg-[#16968F] text-white px-12 py-3 rounded-lg hover:bg-emerald-700 cursor-pointer"
        >
          Add a Skill
        </button>
